...
var (
	...
	defaultProcessorLabels          = append(defaultNodeLabels, "pipeline", "processor")
...
type ingestPipelineProcessorMetric struct {
	Type   prometheus.ValueType
	Desc   *prometheus.Desc
	Value  func(processorsStats NodeStatsIngestPipelinesProcessorsStatsResponse) float64
	Labels func(cluster string, node NodeStatsNodeResponse, pipeline string, processor string) []string
}
...
type Nodes struct {
	...
	ingestPipelineProcessorMetrics []*ingestPipelineProcessorMetric
		...
		ingestMetrics: []*ingestMetric{
			{
				...
				Labels: func(cluster string, node NodeStatsNodeResponse) []string {
					return append(defaultNodeLabelValues(cluster, node))
				},
			},
			{
				Type: prometheus.CounterValue,
		...
		ingestPipelineProcessorMetrics: []*ingestPipelineProcessorMetric{
			{
				Type: prometheus.CounterValue,
				Desc: prometheus.NewDesc(
					prometheus.BuildFQName(namespace, "ingest_pipelines_processor", "time"),
					"Time spent by the processor transforming documents.",
					defaultProcessorLabels, nil,
				),
				Value: func(processorStats NodeStatsIngestPipelinesProcessorsStatsResponse) float64 {
					return float64(processorStats.Time)
				},
				Labels: func(cluster string, node NodeStatsNodeResponse, pipeline string, processor string) []string {
					return append(defaultNodeLabelValues(cluster, node), pipeline, processor)
				},
			},
			{
				Type: prometheus.CounterValue,
				Desc: prometheus.NewDesc(
					prometheus.BuildFQName(namespace, "ingest_pipelines_processor", "count"),
					"Number of documents transformed by the processor.",
					defaultProcessorLabels, nil,
				),
				Value: func(processorStats NodeStatsIngestPipelinesProcessorsStatsResponse) float64 {
					return float64(processorStats.Count)
				},
				Labels: func(cluster string, node NodeStatsNodeResponse, pipeline string, processor string) []string {
					return append(defaultNodeLabelValues(cluster, node), pipeline, processor)
				},
...
func (c *Nodes) Collect(ch chan<- prometheus.Metric) {
	...
	for _, node := range nodeStatsResp.Nodes {
		...
		for pipeline, ppstats := range node.Ingest.Pipelines {
			...
			for _, prstats := range ppstats.Processors {
				for _, metric := range c.ingestPipelineProcessorMetrics {
					ch <- prometheus.MustNewConstMetric(
						metric.Desc,
						metric.Type,
						metric.Value(),
						metric.Labels(nodeStatsResp.ClusterName, node, "hello", "goodbyte")...,
					)
				}
			}
		}
